...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
  if(req.method == 'POST'){//post method as we are getting info
    ...
    if(user){  
      //user.roleID is admin so update the comment and status given by admin
        if(user.roleID === 1){//if user is admin
            const updateC = await prisma.process.update({
              ...
              res.status(200).json({message: `Process ${processID} was updated successfully`, update: updateC})
        }
        //user.roleID is mentor so update the the comment and status given by mentor
        else if(user.roleID === 2){//if user is mentor
            const updateC = await prisma.process.update({
                where:{processID: processID},
                data:{
        ...
        //user.roleID is user
        else if(user.roleID === 3){ // if user is student
          ...
          res.status(200).json({message: `Process ${processID} was updated successfully`, update: updatedProcess})
        }
