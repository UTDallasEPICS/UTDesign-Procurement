...
const AdminRequestCard: React.FC<AdminRequestCardProps> = ({
   ...
   setOrders(reqOrders.length !== 0 ? (
     reqOrders.map((reqOrder, reqOrderIndex) => {
       return {
         ...
         dateOrdered: reqOrder.dateOrdered instanceof Date ? reqOrder.dateOrdered.toISOString() : reqOrder.dateOrdered
       }
     })
   ) : []
   )
 ...
 const handleDeleteItem = (index: number) => {
   if (index < items.length) {
     ...
     setNewItems((prevNewItems) => prevNewItems.filter((_, i) => i !== index - items.length));
   }
 };
 /**
...
*/
 function handleOrderChange(
   ...
   const { name, value } = e.target;
   const updatedValue = name === 'dateOrdered' ? new Date(value).toISOString() : value;
 ...
 const handleDeleteOrder = async (index: number, deletedItem: number) => {
   if (index < orders.length) {
     ...
     setNewOrders((prevNewOrders) => prevNewOrders.filter((_, i) => i !== index - orders.length));
   }
   try {
     const response = await axios.delete(`/api/orders/delete`, { data: { orderNumber: deletedItem } });
     if (response.status === 200) {
 ...
 */
 ...
 async function handleSave() {
   ...
   try {
     ...
     const oldOrderDetails = await axios.get('/api/orders/get/requestOrders/', {
       params: { requestID: details.requestID }
     });
     if (oldOrderDetails.status === 200) {
       const oldOrders = oldOrderDetails.data.orders;
       // Get the orderNumbers of old orders
       const oldOrderNumbers = new Set(oldOrders.map((order: any) => order.orderNumber));
       //check the order is exist to update
       const orderChanged = oldOrders.length === newOrders.length && JSON.stringify(newOrders) !== JSON.stringify(oldOrders);
                 ...
                 <fieldset disabled={!editable}>
                           ...
                           <tr key={itemIndex + items.length}>
                             ...
                             <td>
                               <InputGroup>
                                 <InputGroup.Text>$</InputGroup.Text>
                                 <Form.Control
                                   value={(item.quantity * Number(item.unitPrice)).toFixed(4)}
                                   disabled
                                 />
                               </InputGroup>
                             </td>
                             ...
                             <td>
                               <Button
                                 className={styles.cardBtn}
                                 variant='danger'
                                 onClick={() => handleDeleteItem(itemIndex + items.length)}
                               >
                                 Delete
                               </Button>
                             </td>
                   ...
                   <Row>
                     <Col xs={12} className='d-flex justify-content-end'>
                       {editable && (
                         <Button className={styles.cardBtn} variant='success' onClick={handleAddItem}>
                           Add Item
                         </Button>
                       )}
                     </Col>
                   ...
                   {Boolean(orders.length > 0) && <Table responsive striped>
                     <thead>
                       <tr>
                         <th>Order #</th>
                         <th>Tracking Info</th>
                           ...
                           <tr key={orderIndex}>
                             ...
                             <td>
                               <Form.Control
                                 name='dateOrdered'
                                 type='date'
                                 value={order.dateOrdered ? new Date(order.dateOrdered).toISOString().split('T')[0] : ''}
                                 onChange={(e) => handleOrderChange(
                                   e as React.ChangeEvent<HTMLInputElement>,
                                   orderIndex
                                 )}
                 ...
                 <Row>
                   <Col xs={12} className='d-flex justify-content-end'>
                     {editable && (
                       <Row>
                         <Col>
                           <Button className={styles.cardBtn} variant='success' onClick={handleAddOrders}>
                             Add Order
                           </Button>
                           &nbsp;
                           <Button
